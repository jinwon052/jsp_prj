...
<%
	...
	//=12312&=sdsadasd&=123.png&=5월+1일+근로자의+날+안쉼.&=flag&=JDBC&ta=adsdadasd
	...
	String file=request.getParameter("file");//input type="file" name="file"
	String hid=request.getParameter("hid"); //input type="hidden" name="hid"
	String flag=request.getParameter("flag");//input type="checkbox" name="flag"
	String sel=request.getParameter("sel");//input type="select" name="sel"
	String ta=request.getParameter("ta");//input type="textarea" name="ta"

	/* if("POST".equals(request.getMethod())){//요청방식이 post일 때
		//한글 처리
		txt=new String(txt.getBytes("8859_1"),"UTF_8");
...
<div id="divParam">
	<table class="table table-hover">
		<thead>
			<tr>
				<th colspan="2"> request 내장객체를 사용한 HTML Form Control 입력 값</th>
			</tr>
		</thead>
		<tbody>
			<tr>
			...
			<tr>
				<td>type="hid"</td><td><%= hid %></td>
			</tr>
			<tr>
				<td>type="checkbox"</td><td><%= flag %></td>
			</tr>
			<tr>
				<td>type="select"</td><td><%= sel %></td>
			</tr>
			<tr>
				<td>type="textarea"</td><td><%= ta %></td>
			</tr>
		</tbody>
	</table>
</div>
