...
<style type="text/css">
 ...
 td:hover{font-weight: bold; background-color: #DEE2E6}
</style>
<!-- jquery CDN -->
...
list.add(new DataDTO("img_3.jpg","이건못참지!!!","오전에 수고하셨고 점심 맛있게 드세요","https://yogiyo.co.kr/mobile/#/"));
%>
<div>
<% for(DataDTO dto : list){ %>
<div class="card" style="width: 18rem; float:left; margin-right: 10px;">
  <img style="width: 285px; height: 160px;" src="http://192.168.10.73/jsp_prj/common/images/<%= dto.getImg() %>" class="card-img-top" alt="...">
  <div class="card-body">
...
<%
	...
	LocalDate lastDayOfMonth=ld.withDayOfMonth(ld.lengthOfMonth());
	int nowDay=ld.getDayOfMonth();//현재 날짜 얻기
	int lastDay=lastDayOfMonth.getDayOfMonth();//마지막 날짜 얻기
...
for(int tempDay=1; tempDay < lastDay+1 ; tempDay++){ 
...
%>
	<option value="<%=tempDay %>"<%=selected %>><%= tempDay %> </option>
<%} //end for %>
