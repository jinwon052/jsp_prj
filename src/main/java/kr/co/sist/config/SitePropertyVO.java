...
public class SitePropertyVO {
	...
	@Getter
	private String site_name,stie_version,site_info,protocol,server_name,server_port,servlet_path,logo_img,upload_path;	
	...
	private void setConfig()throws SQLException{
		...
		try{
			...
			String configSql="select site_name,stie_version,site_info,protocol,server_name,server_port,servlet_path,logo_img,upload_path	from site_property";
			rs=stmt.executeQuery(configSql);
			
			if(rs.next()){
				site_name=rs.getString("site_name");
				stie_version=rs.getString("stie_version");
